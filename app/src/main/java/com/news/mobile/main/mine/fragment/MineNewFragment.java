...
public class MineNewFragment extends BaseFragment<MineFPresenter, MineFModel> implements MineFContract.View {
    ...
    public void getPersonMsgSuccess(UserInfoResponse user) {
        tv_login.setText(user.getData().getUserMsg().getNickname().equals("") ? "It's Me" : user.getData().getUserMsg().getNickname());
        ...
        if (user != null) {
            ...
            sv_middle.setCenterString(userMsg.getBalance());
            sv_right.setCenterString(userMsg.getTotal_balance());
        }
