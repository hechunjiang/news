...
import com.news.mobile.entiyt.FacebookRegResponse;
import com.news.mobile.entiyt.LoginUserResponse;
...
import com.news.mobile.main.mine.activity.FindPassActivity;
import com.news.mobile.main.mine.activity.PrivacyActivity;
import com.news.mobile.tplatform.facebook.FacebookPlatform;
import com.news.mobile.tplatform.google.GoogleLogin;
import com.news.mobile.tplatform.twitter.TwitterLogin;
import com.news.mobile.utils.ACache;
import com.news.mobile.utils.AnimationUtils;
        ...
        MyRetrofit.getInstance(mContext)
                .onLogin(request, new DataCallBack() {
                    ...
                    public void onSucceed(String json) {
                        ...
                        mUserSpCache.putString(UserSpCache.KEY_TICKET, user.getData().getTicket());
                        mUserSpCache.putBoolean(UserSpCache.KEY_IS_USER_LOGIN, user.getData().isLogin_flag());
                        mUserSpCache.putString(UserSpCache.KEY_PHONE, request.getAccount());
                        mUserSpCache.putString(UserSpCache.NIKE_NAME, user.getData().getUser_info().getNickname());
                        mUserSpCache.putString(UserSpCache.KEY_PASS, request.getPassword());
                        mUserSpCache.putString(UserSpCache.C_USERID, user.getData().getUser_info().getC_user_id());
                        mUserSpCache.putString(UserSpCache.OPENID, user.getData().getUser_info().getOpenId());
                        mUserSpCache.putString(UserSpCache.KEY_USER, new Gson().toJson(user.getData().getUser_info()));
                        mUserSpCache.putString(UserSpCache.KEY_IS_SECEND_OPEN_APP, "isSecend");
                        mUserSpCache.putUser(user);
                        ACache.get(MyApplication.getAppContext()).put(UserSpCache.KEY_PHONE, request.getAccount(), ACache.STORAGE_TIME);
                        if (mThirdLogin != null) {
